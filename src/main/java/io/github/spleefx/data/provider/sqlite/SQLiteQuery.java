...
public interface SQLiteQuery {
    ...
    /**
     * SQLiteQuery for creating the table.
     */
    ...
    /**
     * SQLiteQuery to update more than 1 entry all at once
     */
    ...
    Function<OfflinePlayer, GameStats> REQUEST_FROM_DATABASE = (player) -> {
        try {
            PreparedStatement statement = SQLiteProvider.SQL.get().prepareStatement(SELECT);
    ...
    static void createTable() {
        try {
            PreparedStatement statement = SQLiteProvider.SQL.get().prepareStatement(CREATE_TABLE);
            statement.execute();
        } catch (SQLException e) {
            throw new DataException(e);
        }
    ...
    static void addAll() {
        ...
        try {
            ...
            DATA_CACHE.forEach((player, statistic) -> builder
                    .append("('").append(DataProvider.getStoringStrategy().apply(player)).append("', '").append(SQLiteQuery.SQL.toJson(statistic)).append("'), "));
            String query = builder.toString().substring(0, builder.length() - 2) + ";";

            /* Execute the statement */
            PreparedStatement statement = SQLiteProvider.SQL.get().prepareStatement(query);
